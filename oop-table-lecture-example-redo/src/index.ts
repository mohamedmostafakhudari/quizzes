...
class Table {
	private employees: Array<EmployeeRow>;
	constructor(initialEmployees: Array<EmployeeRow> = []) {
		this.employees = initialEmployees;
	}
	addRecord(employee: Employee): EmployeeRow {
...
class TableUI {
	static table: HTMLTableElement = document.querySelector("table#employeeTable");
	static tbody: HTMLTableSectionElement = document.querySelector("table#employeeTable tbody");

	static render(employees: Array<EmployeeRow>) {
		TableUI.tbody.innerHTML = "";
		for (const employee of employees) {
			EmployeeRowUI.render(employee);
		}
	}
	static showMessages(messagetext: string, clsName: string) {
...
class EmployeeRow {
	...
	constructor(fname: string, lname: string, salary: string, title: string, department: string) {
...
class EmployeeRowUI {
	...
	static render(employee: EmployeeRow) {
...
function createAlert(messagetext: string, clsName: string): HTMLElement {
...
function resetInputs(formElem: HTMLFormElement) {
...
const formElem: HTMLFormElement = document.querySelector("#addNewEmployeeForm");
...
tableElem.addEventListener("click", (e) => {
	const target = (e.target as HTMLElement).closest("button#deleteEmployee");
	if (!target) return;
